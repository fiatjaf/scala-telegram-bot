...
trait BotAPI[F[_], S[_]] {
  ...
  /** Stream all updated for this bot using long polling. `S[_]` is the
    * streaming effect.
    *
    * @param fromOffset
    *   offset of the fist message to start polling from
    */
...
/** Single bot API instance with http4s client. Requires an implicit decoder for
  * incoming bot updates.
  *
  * @param token
  *   bot api token
  * @param client
...
class Http4SBotAPI[F[_]](token: String, client: Client[F], logger: Logger[F])(
  ...
  private val botApiUri: Uri =
    Uri.uri("https://api.telegram.org") / s"bot$token"
  ...
  private def lastOffset(
      response: BotResponse[List[BotUpdate]]
  ): Option[Offset] =
